...
class ARXInferenceRunner(BaseInferenceRunner):
    """Runner for ARX arm inference tasks.
    ...
    def __init__(self, *args, **kwargs):
        """Initialize the ARX inference runner."""
        if 'camera_names' not in kwargs or kwargs['camera_names'] is None:
            kwargs['camera_names'] = ['cam_top', 'cam_front']

        if 'operator' not in kwargs or kwargs['operator'] is None:
            kwargs['operator'] = {
                'type': 'ARXOperator',
                'img_top_topic': '/camera_top/color/image_raw',
                'img_front_topic': '/camera_front/color/image_raw',
                'arm_status_topic': '/arm_status_joint',
                'arm_cmd_topic': '/arm_cmd',
        ...
        self.prepare_pose = [0.0, -1.57, 1.57, 0.0, 1.57,
                             0.0]  # Default preparation pose
    ...
    def get_ros_observation(
        ...
        Returns:
            Tuple containing:
                ...
                - arm_joint_pos (np.ndarray): Robot arm joint positions
                - arm_gripper (float): Gripper position
        ...
        while not rospy.is_shutdown():
            ...
            print_flag = True
            (img_top, img_front, img_top_depth, img_front_depth, arm_joint_pos,
             arm_gripper, frame_time_min, frame_time_max) = result

            return (img_top, img_front, arm_joint_pos, arm_gripper.data)
    ...
    def update_observation_window(self) -> Dict:
        ...
        img_top, img_front, arm_joint_pos, arm_gripper = (
        ...
        # TODO: Verify joint mapping for ARX arm
        # For UR, it was [2,1,0,3,4,5]. ARX default mapping is [1,2,3,4,5,6]
        # This needs to be verified based on training data
        qpos = np.concatenate([
            np.array(arm_joint_pos)[[0, 1, 2, 3, 4, 5]],
            np.array([arm_gripper])
        ],
                              axis=0)

        observation = {
            'qpos': qpos,
            self.camera_names[0]: img_top,
            self.camera_names[1]: img_front,
        }
    ...
    def _move_to_prepare_pose(self):
        ...
        self.ros_operator.movegrip(0.0)
